...
void reply_rm (accepted_socket& client_sock, cxi_header& header) {
   ...
   outlog << "Removed file: " << header.filename << endl;
...
void reply_put(accepted_socket& client_sock, cxi_header& header){
   ...
   recv_packet(client_sock, buffer.get(), ntohl(header.nbytes));
   ...
   outfile.close();
   buffer.reset();
   outlog << "Recieved file: " << header.filename << endl;
   header.command = cxi_command::ACK;
   header.nbytes = htonl(0);
   memset (header.filename, 0, FILENAME_SIZE);
   send_packet(client_sock, &header, sizeof header);
...
void reply_get(accepted_socket& client_sock, cxi_header& header){
   ...
   int status = stat(&header.filename[0], &stat_buf);
   if(status != 0){
   ...
   outlog << "Sent file: " << header.filename << endl;
}
